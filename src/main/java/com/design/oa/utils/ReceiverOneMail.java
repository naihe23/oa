...
public class ReceiverOneMail {
    private Message[] messages;

    public ReceiverOneMail(Message[] messages) {
     ...
     * 解析邮件
     */
    public List<Map<String, Object>> parseMessage() throws Exception {
        ...
        for (int i = 0, count = messages.length; i < count; i++) {
            ...
            map.put("mailNumber",messages[i].getMessageNumber());
            map.put("mailTopic", getSubject(msg));
            map.put("mailSender", getFrom(msg));
            map.put("mailReceiver", getReceiveAddress(msg, null));
            map.put("mailSendDate", getSentDate(msg, null));
            if (isSeen(msg)) {
                map.put("mailState", "已读");
            } else
                map.put("mailState", "未读");

            map.put("mailFileName", getAttachmentName(msg));
...
//            System.out.println("附件名称：" + getAttachmentName(msg));
...
//                saveAttachment(msg, "D:\\Dowload\\" + msg.getSubject() + "_"); //保存附件
//            }
...
//            System.out.println();
    ...
    public String getReceiveAddress(MimeMessage msg, Message.RecipientType type) throws MessagingException {
        ...
        for (Address address : addresss) {
            ...
            if (!(internetAddress.toUnicodeString().equals("undefined")))
     ...
     * @throws Exception
     */
    public String getAttachmentName(Part part) throws Exception {
    ...
    public boolean isDraft(MimeMessage msg) throws MessagingException {
        return msg.getFlags().contains(Flags.Flag.DRAFT);
    }
    /**
     * 判断邮件是否需要阅读回执
     *
